...
$(function() {
    ...
    $('body').on('touchend', '.suanMyself', function() {
        ...
        $('.answer-content').append('<div class="suan-content-mid"><div class="suan-content-foot"></div></div>');
        $('.answer-content').append('<div class="suan-content-mid"><div class="suan-content-foot"></div></div>');
        a = 2;
        num = 2
        // 展示星盘页面
    ...
    if (ios == -1) {
        ...
        // var divEl3 = document.querySelector('.suan-dialog-page');
        // iosTrouchFn(divEl1, divEl2, divEl3);
    }
    ...
    // 吊起支付弹框
    $('body').on('touchend', '.suan2-pay-bottom,.result-pay-btn', function() {
        if (config_PaySuccess)
            return
        // showDialogAsk();
    ...
    // 阴历阳历切换
    $('body').on('touchend', '.suan-content-mid-data p>span', function() {
    ...
    // 命盘点击切换
    $('.zwResultDiv').on('touchend', '.spanTop,.spanFoot,.spanCenterL>span,.spanCenterR>span', function() {
        $('.spanCenter').css({
    ...
    $('body').on('touchend', '.suan-begin-machine', function(e) {
    ...
    // 继续算算
    $('body').on('touchend', '.suan2-continus', function(e) {
    ...
    // 继续对话
    $('body').on('touchend', '.suan-dialog-ask-answer', function(e) {
        $('.suan-dialog-result').removeClass('init')
        let value = $(this).attr('value');
        let html = $(this).html();
        let $reslut = $('.suan-dialog-result')
        $reslut.append('<p class="answer"><span>'+html+'</span></p>')
        $('.suan-dialog-result').scrollTop(100000);
...
function beginAsk (sendText) {
    ...
    axios({
        ...
        if (responseData.type === 1) {
            ...
            setTimeout(function() {
                $div.find('.loadings').remove();
                $('.suan-dialog-result').append('<p class="question"><span>与你交流的非常愉快，下面是根据你的对话，为你生成的个性化结论，希望我的建议能帮你克服生活中的不顺利，早日完成自己的愿望，每天生活快乐！</span></p>');
                $('.suan-dialog-result').append('<button class="see-result">揭晓</button>');
                $('.suan-dialog-result').scrollTop(100000); 
            }, 1500)  
        }
...
function drawChart () {
    axios({
        ...
        // $('.zwResultDiv .chart_map').html('');
        // $('.zwResultDiv .intro, .zwResultDiv .suan-begin-machine').remove();
        if (response.data.code === 200 && response.data.result) {
            ...
            $('.zwResultDiv .chart_map').append('<span class="spanCenter"><span><span>大师好 ·紫微斗数</span><span>姓名:' + window.userData.name + '&nbsp;' + responseConfig.shadowLight + responseConfig.sex + '&nbsp;' + responseConfig.fiveElement + '</span><span>身主：' + responseData.bodyMaster.name + '&nbsp;&nbsp;' + '命主：' + responseData.destinyMaster.name + '</span><span>农历：' + responseConfig.bornLunar + '</span><span>阳历：' + responseConfig.bornSolar + ' '+ responseConfig.bornTimeGround + '</span></span><br class="clearfloat"></span>');
