...
$(function() {
    ...
    if (storage.getItem('orderId')) {
        ...
        checkOrderSuccess(orderId, function(res) {
            if (config_PaySuccess) {
                let result = JSON.parse(storage.getItem('results'));
                reportInnerHtml(result);
                $('.result').addClass('pay-success').show().siblings().hide();
            }
        });
    } else {
    ...
    $('body').on('click', '.suan-content-foot-chart', function() {
        _hmt.push(['_trackEvent', 'button', 'sendShowStarChart']);
        // 直接显示命盘页面即可
    ...
    $('#zwResultDiv').on('click', '.back-btn', function() {
        ...
        _hmt.push(['_trackEvent', 'button', 'starChartBack']);
    })
    ...
    $('body').on('click', '.suan-begin-machine', function(e) {
        a++
        num++
        $('.suan0').find('.suan-content-mid').hide()
        $('#zwResultDiv').hide();
        pushData(a, num, $(this).attr('value'));
        $('.suan0').show().siblings().hide()
    })
...
function beginAsk (sendText) {
    ...
    axios({
        ...
        if(responseData.type === 0) {
            $('.suan-content-mid').eq(num).prepend(Mustache.render(htmlList.data4, {
                'text': responseData.starContent ? responseData.starContent : '' + '' + responseData.content ? responseData.content : ''
            }))
            let _typeList = responseData.optionList;
            for (let i = 0; i < _typeList.length; i++) {
                $('.suan-content-mid').eq(num).children('.suan-content-foot').append(Mustache.render(htmlList.btn_askBtn, {
                    'text': _typeList[i].showTxt,
                    'value': _typeList[i].requestValue
                }))
            }
            $('.suan-content-mid').eq(num).children('.suan-content-foot').append(Mustache.render(htmlList.btn_chart, {
                'text': '查看命盘'
            }))
        }
        ...
        if (responseData.type === 1) {
            ...
            setTimeout(function() {
                ...
                // 把结论写入本地历史记录
                let results = {
                    results: responseData.chatReport ?  responseData.chatReport : ''
                }
...
function reportInnerHtml(msg) {
    ...
    $('.suan2-title').html(Mustache.render($('#jTmpl_result_title').html(), {
        name: storage.getItem('name') || '',
        isSolarDate: storage.getItem('isSolarDate') === '1' ? '阳历' : '阴历', // 是否阳历 1阳历 0阴历
        birth: storage.getItem('birthDate') + ' ' + storage.getItem('birthHour')
    }));
...
function payForWechat(clientType, openid) {
    //alert(getRequest() && getRequest().code);
    let requetData = {
        amount: 0.01,
...
// 检查订单是否支付成功
function checkOrderSuccess(orderId, fn) {
